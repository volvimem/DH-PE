...
if(!db) {
    for(let key of OLD_KEYS) {
        ...
        if(oldData) { try { db = JSON.parse(oldData); console.log("Migrado de " + key); break; } catch(e) {} }
    }
}
if(!db) db = DEFAULT_DB;
...
function restoreInputs() {
    document.querySelectorAll('input, select, textarea').forEach(el => {
        if(el.id && localStorage.getItem('autosave_' + el.id)) el.value = localStorage.getItem('autosave_' + el.id);
    });
}
...
function mascaraCPF(i) { 
    ...
    if(i.classList) { if(validarCPF(i.value)) { i.classList.add('valid'); i.classList.remove('invalid'); } else { i.classList.add('invalid'); i.classList.remove('valid'); } }
    saveInput(i); 
}
...
function toast(m) { const t=document.getElementById('toast'); if(t) { t.innerText=m.toUpperCase(); t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),3000); } }
...
function loadDarkMode() { const isDark = localStorage.getItem(DARK_MODE_KEY) === 'true'; if(isDark) document.body.classList.add('dark-mode'); }
...
function nav(t) {
    ...
    if(t === 'adm') { 
        if(loggedUser.role === 'ADMIN' || loggedUser.role === 'ORGANIZER') { 
            ...
            const lastAdm = localStorage.getItem(LAST_ADM_KEY); if(lastAdm) openAdmSection(lastAdm); else backToAdmMenu(); 
    ...
    } else { localStorage.removeItem(LAST_ADM_KEY); renderContent(t); }
}
...
function openAdmSection(sec) {
    ...
    if(sec === 'events') { 
        document.getElementById('adm-sec-events').style.display='block'; refreshAdmLists(); restoreEventForm();
...
function saveGlobalConfig() { db.config.phone = document.getElementById('adm-cfg-phone').value; saveDB(); toast("DADOS SALVOS!"); }

function copyPointsFrom(evtId) {
    if(!evtId) return; const evt = db.events.find(e => e.id == evtId);
    if(evt && evt.points) { evt.points.forEach((p, i) => { const el = document.getElementById('pt-'+i); if(el) el.value = p; }); saveEventForm(); toast("PONTUAÇÃO COPIADA!"); }
}
...
function deleteUser(cpf, nome) {
    if(confirm("ATENÇÃO: TEM CERTEZA QUE DESEJA EXCLUIR " + nome + "?\n\nIsso apagará o usuário, tempos lançados e a posição no ranking.")) {
        ...
        saveDB(); filterPilots('edit-user', true); toast("ATLETA E DADOS REMOVIDOS!");
    }
}
...
function selectPilot(cpf, name, ctx) {
    if(ctx === 'res') { 
        ...
        document.getElementById('adm-res-search').value = ''; document.getElementById('adm-res-list').style.display = 'none'; saveAdmState();
    } 
...
function promoteToOrganizer() {
    const cpf = document.getElementById('adm-org-selected-cpf').value; if(!cpf) return toast("SELECIONE UM ATLETA");
...
function demoteOrganizer(cpf) {
    if(confirm("REMOVER PERMISSÃO DE ORGANIZADOR?")) { const idx = db.users.findIndex(u => u.cpf === cpf); if(idx > -1) { db.users[idx].role = 'USER'; db.users[idx].cat = "RÍGIDA"; db.users[idx].allowedEvts = []; saveDB(); renderOrgList(); } }
}
...
let currentFilterStatus = 'ALL'; function filterFinList(status) { currentFilterStatus = status; renderInscriptions(); }
...
function openPrintOptions() {
    const evtId = document.getElementById('fin-evt-select').value; if(!evtId || evtId === 'ALL') return toast("SELECIONE UM EVENTO ESPECÍFICO");
...
function openShareModal(evtName) { document.getElementById('share-piloto-name').innerText = loggedUser.nome; document.getElementById('share-event-name').innerText = evtName; document.getElementById('modal-share').style.display = 'flex'; }
...
function addEvent() { 
    ...
    const finalizeSave = (imgData) => {
         if(t && d && c) { 
            if(idEdit) { const idx = db.events.findIndex(e => e.id == idEdit); if(idx > -1) { db.events[idx] = { ...db.events[idx], t, m, d, city:c, val:v, points: newPoints, status: status, closeDate: closeDate, pix: pix }; if(imgData) db.events[idx].img = imgData; toast("ATUALIZADO"); } } 
            else { const newEvt = {id:Date.now(), t, d, m, city:c, val:v, est:document.getElementById('adm-evt-cb-pe').checked, cbc:document.getElementById('adm-evt-cb-cbc').checked, open:true, img: imgData, points: newPoints, status: status, closeDate: closeDate, pix: pix}; db.events.push(newEvt); toast("CRIADO"); } 
...
function addResult() { 
    ...
    if(!idx && db[type].some(x => x.evtId == evtId && x.name == piloto.nome)) { return toast("PILOTO JÁ LANÇADO!"); }
    ...
    limparFormResultados(); saveDB(); refreshAdmLists(); 
}
function approveResult(type, index) { db[type][index].status = 'OK'; if(type === 'tempos') recalcRankingFromTimes(db[type][index].evtId); saveDB(); refreshAdmLists(); toast("APROVADO!"); }
...
function delItem(col, idx) { if(confirm("APAGAR?")) { const item = db[col][idx]; db[col].splice(idx, 1); if(col === 'tempos') recalcRankingFromTimes(item.evtId); saveDB(); refreshAdmLists(); } }
function maskTimeOrPoints(i, e) { if(document.getElementById('adm-res-type').value === 'ranking') return; if(e && e.inputType === 'deleteContentBackward') return; let v = i.value.replace(/\D/g, ""); if (v.length > 7) v = v.substring(0, 7); if (v.length > 5) v = v.substring(0, 5) + "." + v.substring(5); if (v.length > 2) v = v.substring(0, 2) + ":" + v.substring(2); i.value = v; }
...
function fazerLogin() { 
    try {
        ...
        if(user) { loggedUser = user; localStorage.setItem(SESS_KEY, JSON.stringify(user)); if(remember) localStorage.setItem(REMEMBER_KEY, cpfRaw + "|" + pass); initApp(); } else { toast("DADOS INCORRETOS"); }
...
function abrirRecuperacao() { document.getElementById('rec-cpf').value = ''; document.getElementById('rec-tel').value = ''; document.getElementById('rec-answer').value = ''; document.getElementById('rec-security-area').style.display = 'none'; document.getElementById('btn-buscart-rec').style.display = 'block'; document.getElementById('modal-recovery').style.display = 'flex'; }
...
function buscarUsuarioRecuperacao() {
    ...
    if(user && cleanCPF(user.tel) === cleanCPF(tel)) {
        ...
        let qText = questions[user.secQ] || "Pergunta de Segurança"; if(!user.secQ) { alert("SUA SENHA É: " + user.pass); fecharModal('modal-recovery'); return; }
...
function initApp() { 
    ...
    loadDarkMode(); trocarTela('app'); const lastTab = localStorage.getItem(LAST_TAB_KEY);
...
document.addEventListener('DOMContentLoaded', () => {
    restoreInputs(); document.querySelectorAll('input, select, textarea').forEach(el => { el.addEventListener('input', () => saveInput(el)); el.addEventListener('change', () => saveInput(el)); });
});
