// --- IMPORTAÇÕES FIREBASE (NÃO APAGUE) ---
    ...
    if(t === 'calendar') {
        ...
        const createCard = (e, isHighlight) => {
            ...
            if(loggedUser) {
                ...
                if(ins && ins.status === 'CONFIRMADO') actionBtn = `<button class="btn-share" onclick="openShareModal('${e.t}')"><i class="fas fa-instagram"></i> TICKET</button>`;
        ...
        div.innerHTML = filtered.map((r, i) => {
            ...
            return `<div class="rank-row"><div class="rank-pos">${i+1}</div><div class="rank-name">${r.name}<span class="rank-cat">${r.cat}</span></div><div class="rank-val">${r.val}</div></div>`;
        }).join('');
    }
};
...
window.filterPilots = (ctx) => {
    ...
    listDiv.innerHTML = '';
    if(text.length < 2) { if(ctx!=='edit-user') listDiv.style.display = 'none'; return; }
